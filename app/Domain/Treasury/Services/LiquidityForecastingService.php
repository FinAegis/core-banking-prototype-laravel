...
class LiquidityForecastingService
{
    ...
    private function calculateConfidenceLevel(Collection $historicalData, array $patterns): float
    {
        ...
        $patternStrength = 0;
        if (!empty($patterns['seasonal_patterns'])) $patternStrength += 0.25;
        if (!empty($patterns['day_of_week_patterns'])) $patternStrength += 0.25;
        if (abs($patterns['trend'] ?? 0) < 0.5) $patternStrength += 0.25; // Stable trend
        if (($patterns['inflow_volatility'] ?? 1) < 0.3) $patternStrength += 0.25; // Low volatility
